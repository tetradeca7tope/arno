...
def KernelSmoothing(name, dataset, bw_method=None, lower=float('-inf'),
upper=float('inf'), observed=False, value=None):
    '''Create a pymc node whose distribution comes from a kernel smoothing
density estimate.'''
...
for i in range(10):
    ...
    OBS = pymc.Normal('OBS', Y, 0.1, value=xtrue*xtrue+pymc.rnormal(0,1),
observed=True)
