...
!       print *,'Scaling factor a =',a
      ...
      write(*,'(a27,f12.8)') 'Chisq', chi2
      write(*,'(a27,f12.8)') 'SDSS LRG log-likelihood is ',lnL
      write(*,'(a27,f12.8)') 'The answer should be       ',-9.59273150d0
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      print *,kbands,' k-bands loaded from ',fname(1:lnblnk(fname))
...
555   read(2,*,end=666) (tmp(i),i=1,5)
      ...
      goto 555
666   close(2)
      print *,bands,' measurements loaded from ',fname(1:lnblnk(fname))
      ...
      print *,bands,' measurements will be used.'

      fname = 'sdss_lrg_windows.txt'
      call LoadMatrix(maxbands,bands,kbands,W,fname)
      print *,bands,' window functions loaded from ',fname(1:lnblnk(fname))
...
!     a_angular = (Om/0.25)**(-0.065) * (-w*Otot)**0.14 ! Approximation based on Taylor expansion
      ...
      real*8 Or, Ok, Ox, w
      common/eta/Or, Ok, Ox, w
      real*8 Omega_r, Omega_k,Omega_x,w_eos, z, eta, epsabs, epsrel, amin, amax
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      call compute_EisensteinHu_dewiggled_power(p,kstar,kbands,kvec,power)
      ...
      implicit none
      real*8 p(7), kstar, As, Ok, Ol, Om, Okz, Olz, Omz, z, E, g, f, D, s
      Ok    = p(1)
      Ol    = p(2)
      !oc   = p(3)
      !ob   = p(4)
      ...
      subroutine compute_EisensteinHu_dewiggled_power(p,kstar,bands,kvec,power)
...
!     implicit real*8 (a-h,k,o-z)
      ...
      real*8 transfer_wiggles, transfer_nowiggles, TF_nowiggles, wiggleweight, x
      ...
      call TFtransfer_function(kk,omhh,f_baryon,tf_full,tf_baryon,tf_cdm)
      ...
      transfer = wiggleweight*transfer_wiggles + (1-wiggleweight)*transfer_nowiggles
...
c  Main Variables
     ...
     &           (1.-tf_cdm)*TF_pressureless(q,alpha_c,beta_c)
            ...
            tf_baryon = TF_pressureless(q,1.d0,1.d0)/(1.d0+(ks/5.2d0)**2)
