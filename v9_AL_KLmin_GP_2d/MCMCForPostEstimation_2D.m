...
% PLOT_FUNC(Th1, Th2, True_post, 'Color', 'g');
...
for mcmc_run_iter = 1:0 % NUM_MCMC_RUNS
  ...
  for sample_iter = 1:NUM_MCMC_SAMPLES
    ...
    if strcmp(MEAN_FUNC, 'lowest')
      mcmc_hyper_params.meanFunc = @(arg) min(LOWEST_VAL, min(all_log_joints));
    else
    ...
    [est_joint_log_probs, K, gpSmOpt, gpPrOpt] = GPKFoldCV(all_pts, ...
      all_log_joints, th, NUM_KFOLDCV_PARTITIONS, candidates,mcmc_hyper_params);
